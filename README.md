...
| `Dismount-LLMStorage` | *None* | **Yes** | Refuses to detach the VHDX while fresh shared session leases exist, unless `-Force` is used. |
| `Get-LLMStorageStatus` | *None* | No | Queries storage telemetry and reports the count of active shared session leases. |
| `Get-ActiveLLMSessionLeases` | *None* | No | Lists fresh shared LLM session leases stored on the mounted LLM volume. |
...
* **Lease-Based Idle Session Protection**: Each interactive `start-llm` prompt creates a shared lease in `M:\.ollama-control\sessions` / `/mnt/wsl/models_storage/.ollama-control/sessions`. This keeps Ollama online even when an idle prompt has no established TCP socket.
* **Socket-Based Active Request Protection**: The `stop-llm` teardown block still uses `ss` to detect active or connecting requests on port `11434`, treating `ESTAB` and `SYN-RECV` as keep-alive states. `TIME-WAIT` alone does not keep Ollama alive.
...
## Shared Session Manager Notes

The VHDX-mounted LLM volume now stores both model data and runtime coordination state. Existing model data remains at the volume root. The scripts add a hidden control directory:
...
The cleanup rule is intentionally stricter than socket-only cleanup:

```text
safe to stop Ollama =
  LLM volume mounted
  AND no ESTAB/SYN-RECV sockets on port 11434
  AND no fresh shared session leases
```

This fixes the idle-prompt case where a WSL terminal is still sitting at the LLM prompt but no established socket is visible to `ss`.


== WSL PID Namespace Note

The shared session manager treats `SESSION_ID` as an application-level lease identity, not as a Windows or WSL session identifier. The lease manager creates it so the shared control directory has a stable primary key.

Linux PIDs are used only as liveness evidence. WSL2 can involve multiple PID namespaces, so a PID observed inside one distribution or shell namespace is not assumed to be globally meaningful by itself. Lease files therefore include:

* `SESSION_ID` — logical lease identity generated by the lease manager
* `DISTRO` — owning WSL distribution name
* `LOCAL_PID` — PID observed by the owning shell namespace
* `ROOT_NAMESPACE_PID` — first `NSpid` value when available
* `PID_NAMESPACE` — namespace inode reported by `/proc/<pid>/ns/pid`

Cleanup validates `LOCAL_PID` only when it is running in the same PID namespace as the lease owner. Otherwise, heartbeat freshness is the safe cross-distribution fallback.
