...
_llm_pid_namespace() {
    readlink /proc/self/ns/pid 2>/dev/null || printf 'unknown\n'
}

_llm_process_pid_namespace() {
    local pid="$1"
    readlink "/proc/${pid}/ns/pid" 2>/dev/null || printf 'unknown\n'
}

_llm_root_namespace_pid() {
    local pid="${1:-$$}"
    local nspid_line

    nspid_line="$(awk '/^NSpid:/ { $1=""; sub(/^ /, ""); print; exit }' "/proc/${pid}/status" 2>/dev/null || true)"

    if [ -n "$nspid_line" ]; then
        # NSpid is ordered from the outer/root PID namespace to the innermost
        # namespace where the process is being observed.
        printf '%s\n' "$nspid_line" | awk '{ print $1 }'
        return 0
    fi
...
_llm_new_session_id() {
    local distro now random_part
    distro="$(_llm_distro_name | tr -c '[:alnum:]_.-' '-')"
    ...
    random_part="${RANDOM}${RANDOM}${RANDOM}"
    printf '%s-%s-%s\n' "$now" "$distro" "$random_part"
}
...
_llm_write_lease() {
    ...
    local distro="$3"
    local local_pid="$4"
    local now_epoch now_iso pid_namespace root_namespace_pid parent_pid

    now_epoch="$(_llm_now_epoch)"
    now_iso="$(_llm_now_iso)"
    pid_namespace="$(_llm_process_pid_namespace "$local_pid")"
    root_namespace_pid="$(_llm_root_namespace_pid "$local_pid")"
    parent_pid="${PPID:-}"
...
HOSTNAME='$(hostname)'
LOCAL_PID='$local_pid'
ROOT_NAMESPACE_PID='$root_namespace_pid'
...
_llm_acquire_lease_locked() {
    ...
    session_id="$(_llm_new_session_id)"
...
_llm_reap_stale_leases_locked() {
    local now lease_file last_seen local_pid pid_namespace current_pid_namespace age

    now="$(_llm_now_epoch)"
    current_pid_namespace="$(_llm_pid_namespace)"
    ...
    for lease_file in "$SESSIONS_DIR"/*.lease; do
        ...
        local_pid="${LOCAL_PID:-${PID:-}}"
        pid_namespace="${PID_NAMESPACE:-}"
        age=$((now - last_seen))

        # A namespace-local PID can only be validated from the same PID namespace.
        # If this process is in the same namespace as the lease owner, kill -0 is
        # strong liveness evidence. Otherwise, the heartbeat TTL is the portable
        # cross-distro fallback.
        if [ -n "$local_pid" ] && [ -n "$pid_namespace" ] && [ "$pid_namespace" = "$current_pid_namespace" ] && kill -0 "$local_pid" 2>/dev/null; then
            continue
        fi
